...
`copy path` / `copy anchor` / `copy diff` / `copy notes` / `copy notes QUERY` / `copy prompt` / `copy prompt file` 会把复制文本传给 copy 命令的 stdin，也支持 `{text}` 占位。`save diff` / `save prompt` / `save prompt file` 直接写入文件，不依赖剪贴板命令。`reveal` 支持 `{file}` 和 `{dir}` 占位。对应环境变量是 `CR_COPY_CMD` 和 `CR_REVEAL_CMD`。
