...
- `cr browse`
  ...
  - Supports keyboard navigation with arrows or `j/k`, Enter or right arrow to open a file, `n/p` for next/previous file, `]` / `[` for next/previous File Detail hunk, `b` or left arrow to return through in-session page history, `forward` to move forward after back, `r` to refresh, and `q` to quit.
...
- Browse:
  ...
  - Treat File Detail hunk navigation as its own UI model: rendered hunk header detection and next/previous scroll selection live in `cr.ui.file_detail_navigation`. `BrowserCommandExecutor` applies the returned scroll and status message; Page Content keeps rendering File Detail lines.
