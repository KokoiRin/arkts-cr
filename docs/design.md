...
- Browse:
  ...
  - Treat File Detail navigation as its own UI model: rendered hunk header detection, next/previous scroll selection, active hunk line resolution, active rendered hunk extraction, and rendered text find live in `cr.ui.file_detail_navigation`. `BrowserCommandExecutor` applies returned scroll/status for hunk navigation and find; selected-file hunk open/copy workflows live in `cr.ui.selected_file_actions`; Page Content keeps rendering File Detail lines.
