...
- Browse:
  ...
  - Treat File Detail navigation as its own UI model: rendered hunk header detection, next/previous scroll selection, active hunk line resolution, active rendered hunk extraction, current rendered row new-line resolution, rendered text find, and repeated find-match navigation live in `cr.ui.file_detail_navigation`. `BrowserCommandExecutor` applies returned scroll/status for hunk navigation and find; selected-file hunk/line open/copy workflows live in `cr.ui.selected_file_actions`; Page Content keeps rendering File Detail lines.
