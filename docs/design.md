...
- `cr browse`
  ...
  - Provides `problems` / `task problems` as a lightweight Problems page over
    ...
    Page `copy source` copying nearby source context, `copy problem context`
...
- Browse:
  ...
  - Treat Source File Page as a cross-layer read-only preview: `cr.ui.source_file` reads repo-local UTF-8 files, computes target-line windows, and formats target-line source context handoff; Page Content renders rows and active copied-context radius; Browser Navigation owns page-local source path, target line, scroll, find text, and context radius snapshots; Browser Action Execution owns navigation, source-page-local find state, clipboard side effects, context-radius command handling, and editor handoff. It is not File Detail, does not search across files, and does not enter workspace persistence.
