...
- Browse:
  ...
  - Treat File Detail navigation as its own UI model: rendered hunk header detection, next/previous scroll selection, changed-row detection and navigation, active hunk line resolution, active rendered hunk extraction, current rendered row new-line resolution, rendered text find, and repeated find-match navigation live in `cr.ui.file_detail_navigation`. `BrowserCommandExecutor` applies returned scroll/status for hunk/change navigation and find; selected-file hunk/line open/copy workflows live in `cr.ui.selected_file_actions`; Page Content keeps rendering File Detail lines.
  - Treat `Selected File Actions` as the owner of workflows that act on the current Changed Files selection: open, open/copy hunk, open/copy line, copy path, copy anchor, copy/save diff snippet, reveal, stage/unstage, selected-file notes, and selected/scope prompt handoff selection live in `cr.ui.selected_file_actions`. `BrowserCommandExecutor` routes parsed actions, displays returned messages, and refreshes Changed Files after successful index mutations; `cr.ui.file_actions` keeps platform subprocess details, while `cr.vcs.git` keeps Git index subprocess details.
