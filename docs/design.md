...
- `cr browse`
  ...
  - Updates task output by repainting only the task panel when the user is idle
    on ordinary pages.
  - Provides a `task output` / `output` page for reading the current task's
    captured output with independent scrolling; that page redraws main content
  ...
  - Provides `problems` / `task problems` as a lightweight Problems page over
    ...
    filters and text queries narrowing the visible Problems list, optional severity sorting,
    optional file grouping for scanning,
    ...
    File Page `source context N` adjusting copied context radius, Source File
    Page `source select START END` selecting an exact source range, Source File
    Page `source mark` / `source select to` selecting from a marked source line
    to the current target line, Source File Page `copy source` copying the active selected range or nearby source context,
    `copy problem context`
    copying focused problem/source/diff handoff, `save problem context` writing
    the same focused handoff to disk, and `copy problem` /
    `copy problems` / `copy file problems` copying selected/visible/same-file
    problem handoff text, and `next problem file` / `prev problem file`
    jumping between visible problem files.
  ...
  - Lets users copy or save the current task output with `copy task` and `save task [PATH]`, defaulting saved output to `.cr/handoff/task-output.md`; `copy task tail [N]` and `save task tail [PATH]` hand off a compact output tail without changing task history or capture behavior.
  ...
  - Supports keyboard navigation with arrows or `j/k`, Enter or right arrow to open a file, `n/p` for next/previous file, `]` / `[` for next/previous File Detail hunk, `next change` / `prev change` for next/previous added/deleted rows, `copy change` for copying the current added/deleted row as compact review context, `note change TEXT` for appending a per-file review note anchored to the current added/deleted row, `find TEXT` plus `next match` / `prev match` for searching the current rendered File Detail text, `open hunk` for editor handoff to the active hunk, `copy hunk` for copying the active rendered hunk as compact review context, `open line` / `copy line` for the current rendered File Detail row's new-file anchor, `copy line` for the current Source File Page target anchor, `b` or left arrow to return through in-session page history, `forward` to move forward after back, `r` to refresh while preserving File Detail when the selected file survives, and `q` to quit.
  ...
  - Supports selected-file diff snippet copy/save with `copy diff` and `save diff [PATH]`, producing compact Markdown for the current file's review facts and hunks without the full AI prompt wrapper.
...
- Browse:
  ...
  - Treat `BrowserPage` as the canonical internal page vocabulary for Scope Home, Commit Picker, Changed Files, File Detail, Command Palette, Task Output, Task Problems, and Source File.
  ...
  - Treat `ReviewWorkspace` as the owner of changed-file reloads that preserve selected paths; `browser.py` decides whether a refreshed page can remain in File Detail and clamps File Detail scroll after cache invalidation.
  - Treat `Page Content` as the owner of browser page main-content rendering: prompt labels, help lines, contextual action bar text, scope breadcrumbs/context, Scope Home entries and count display, Changed Files tree rows, local change source badges and source summaries, Commit Picker rows and change summaries, empty states, File Detail lines, File Detail review queue dock lines, and scroll-window calculations live in `cr.ui.page_content`. `browser.py` adapts live state to those helpers and keeps frame placement, command execution, selected-file side effects, and session startup/shutdown.
  ...
  - Treat File Detail navigation as its own UI model: rendered hunk header detection, next/previous scroll selection, changed-row detection/navigation, current changed-row extraction, active hunk line resolution, active rendered hunk extraction, current rendered row new-line resolution, rendered text find, and repeated find-match navigation live in `cr.ui.file_detail_navigation`. `BrowserCommandExecutor` applies returned scroll/status for hunk/change navigation and find; selected-file hunk/line/change open/copy/note workflows live in `cr.ui.selected_file_actions`; Page Content keeps rendering File Detail lines.
  - Treat `Selected File Actions` as the owner of workflows that act on the current Changed Files selection: open, open/copy hunk, open/copy line, copy change, note current change, copy path, copy anchor, copy/save diff snippet, reveal, stage/unstage, selected-file notes, and selected/scope prompt handoff selection live in `cr.ui.selected_file_actions`. `BrowserCommandExecutor` routes parsed actions, displays returned messages, and refreshes Changed Files after successful index mutations; `cr.ui.file_actions` keeps platform subprocess details, while `cr.vcs.git` keeps Git index subprocess details.
  ...
  - Treat the contextual action bar as main-content guidance: it follows the scope/status line, changes by BrowserPage, uses existing command names only, and never becomes persisted workspace state.
  ...
  - Treat task output handoff as Task Runtime text plus UI-edge delivery: `cr.ui.tasks` renders the current task output Markdown and compact tail Markdown, while Browser Action Execution calls clipboard or handoff-file helpers. Task output handoff does not enter workspace persistence or task history.
  ...
  - Treat Task Problems as a lightweight UI projection of current task output: `cr.ui.task_problems` extracts repo-local file anchors plus generic severity/code/message facts, filters by severity, filters by plain text query, optionally sorts the visible list by severity, formats visible severity counts, and formats problem handoff text; Page Content renders the list and optional file-group headers; Browser Navigation owns page-local `problem_filter`, `problem_query`, `problem_sort`, and `problem_group` with selection/scroll snapshots; Browser Action Execution owns selection plus editor/copy side effects, including selected-problem, full-visible-list, visible same-file handoff, and visible file-to-file selection jumps.
  - Treat Source File Page as a cross-layer read-only preview: `cr.ui.source_file` reads repo-local UTF-8 files, computes target-line windows, marks selected rows, and formats target-line context or selected-range source handoff; `cr.source.outline` owns best-effort current-symbol lookup; Page Content renders rows plus active copied-context radius, selected range, mark line, and derived symbol label; Browser Navigation owns page-local source path, target line, scroll, find text, context radius, selection, and mark snapshots; Browser Action Execution owns navigation, source-page-local find state, clipboard side effects, context-radius/selection/mark command handling, symbol metadata handoff, and editor handoff. It is not File Detail, does not search across files, and does not enter workspace persistence.
  - Treat problem-context handoff as a focused UI composition over existing facts: `cr.ui.problem_context` owns Markdown assembly only; Browser Action Execution gathers the selected Task Problem or Source File Page target, asks `cr.ui.source_file` for source context, asks `cr.review.data` / `cr.review.snippet` for same-file diff when the file exists in the current Review Scope, then copies through existing clipboard handling or saves through `cr.ui.handoff`. It is not a diagnostics database, prompt template, task transcript, batch export, or persisted problem store.
  ...
  - Treat selected-file diff snippets as a lighter `cr.review` output format reused by the browser: `copy diff` and `save diff` choose the current visible file through `cr.ui.selected_file_actions`, build structured review data for that one file, and render Markdown through `cr.review.snippet`. UI-side saved diff file paths and writes stay in `cr.ui.handoff`.
  - Keep progress commands simple and reversible: `m`/`seen` marks the current file, `done next` / `seen next` marks the current file and advances to the next visible file, `todo`/`unseen` clears it, `remaining` filters to unreviewed files, and `allfiles` returns to the full changed-file list.
