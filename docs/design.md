...
- `cr browse`
  ...
  - Provides `problems` / `task problems` as a lightweight Problems page over
    ...
    copying focused problem/source/diff handoff, and `copy problem` /
...
- Browse:
  ...
  - Treat problem-context handoff as a focused UI composition over existing facts: `cr.ui.problem_context` owns Markdown assembly only; Browser Action Execution gathers the selected Task Problem or Source File Page target, asks `cr.ui.source_file` for source context, asks `cr.review.data` / `cr.review.snippet` for same-file diff when the file exists in the current Review Scope, then copies through existing clipboard handling. It is not a diagnostics database, prompt template, task transcript, or save-file workflow.
