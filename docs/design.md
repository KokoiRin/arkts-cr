...
- `cr browse`
  ...
  - Supports lightweight per-file review notes with `note TEXT` to set a note, `note` to clear it, `notes` to summarize all current workspace notes, `notes QUERY` to filter notes by path/text, and `copy notes` / `notes copy` to copy the full summary; noted files are marked in Changed Files and shown in File Detail.
...
- Browse:
  ...
  - Treat in-session review notes as browser workspace state: path-keyed notes belong in `ReviewWorkspace`, are edited through `note TEXT` / `note`, summarized/searched through `notes`, copied through `copy notes`, and persist with `.git/cr/browse-state.json`.
