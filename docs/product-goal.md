...
3. Source/File Detail 阅读体验
   - Status: first slices implemented for Source File current-symbol hints, symbol range selection, File Detail source bridge, direct source copy, and direct symbol copy.
   - 更好的源码范围选择。
   - 当前函数/符号提示。
   - 复制函数块或相关上下文。
...
## 可直接用于 Codex goal 的提示词

```text
持续推进 cr。

cr 是 terminal-first 的 AI change workbench，不是完整替代 IDE。它服务的是“AI 写完代码后，人类接管、理解、验证、继续交给 AI 修”的高频流程。核心 95% 需求是：查看变更、查看文件、阅读代码、简单跳转、偶尔执行 build/test/lint、查看日志/Problems、复制最小必要上下文给 AI 或 reviewer。

每一轮先检查当前仓库状态、README、docs/product-goal.md、docs/p0.md 和相关代码，然后自己选择一个小到中等、最能提升核心流程的 P0。优先服务这条主线：

AI 改完代码 -> 查看变更范围 -> 查看改了哪些文件 -> 逐文件阅读 diff / 源码 -> 在文件、hunk、改动行、问题之间跳转 -> 偶尔运行 build/test/lint -> 查看 task output / Problems -> 复制或保存问题 + 源码 + diff 上下文给 AI -> 标记已看、写简单备注、继续下一轮。

执行方式：OpenSpec 设计 -> TDD 实现 -> 测试验证 -> Warden 边界审查 -> 文档/P0 更新 -> 提交并推送。保持架构可演进：core/domain 负责事实和规则，TUI 负责展示和输入，外部动作放边界。不要把具体仓库特例塞进公共层，不要过早引入完整 GUI、语言服务或复杂状态机。完成一个 P0 后不要结束 goal，继续按这个目标挑下一个 P0。
```
