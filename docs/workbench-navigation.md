...
File Detail 是三级对象：它展示某个文件在当前 Review Scope 中的具体变化。底部紧凑 changed-file 队列用于在阅读 diff 时保留当前 Changed Files 上下文。
...
### P0: File Detail review queue dock

Status: implemented.

File Detail now renders a compact Changed Files dock at the bottom when enough vertical space is available. The dock shows the selected file position, nearby visible changed files, seen/todo state, note marker, source label, and change summary. It reuses existing Changed Files / workspace facts and does not introduce an independent dock selection, persistence state, mouse interaction, or pane focus model. When vertical space is tight, File Detail keeps the diff readable and omits the dock.

### P0: File Detail hunk navigation

Status: implemented.

`next hunk` / `]` and `prev hunk` / `[` now move within the current File Detail diff by jumping between rendered hunk headers. `next change` and `prev change` jump between actual added/deleted rows inside the current File Detail. `copy change` copies the current actual changed row as a compact single-row review snippet; added rows include a `path:line` anchor, while deleted rows include only the old line number. `note change TEXT` appends the current actual changed row position into the selected file's per-file review note, using `line N: TEXT` for added rows and `old line N: TEXT` for deleted rows. `find TEXT` searches the current rendered File Detail text and jumps to the first match; `next match` / `prev match` repeat that last non-empty query with wraparound. `open hunk` opens the active hunk's new-file line in the editor through the existing open command configuration. `copy hunk` copies the active rendered hunk as line-numbered Markdown review context. `open line` opens the current rendered File Detail row's new-file line, and `copy line` copies that row's `path:line` anchor. These commands stay inside the selected file and preserve Review Scope, filters, notes, progress, and task state. `cr.ui.file_detail_navigation` owns hunk header detection, target scroll calculation, changed-row detection/navigation, current changed-row extraction, active hunk line resolution, active hunk extraction, current row new-line resolution, rendered text find, and repeated match navigation. `cr.ui.selected_file_actions` owns selected hunk/line/change open/copy/note workflows, while `BrowserCommandExecutor` keeps page checks, cached File Detail line retrieval, status feedback, and redraw control.
...
### P0: Commit Picker change summary
