...
`next hunk` / `]` and `prev hunk` / `[` now move within the current File Detail diff by jumping between rendered hunk headers. `next change` and `prev change` jump between actual added/deleted rows inside the current File Detail. `find TEXT` searches the current rendered File Detail text and jumps to the first match; `next match` / `prev match` repeat that last non-empty query with wraparound. `open hunk` opens the active hunk's new-file line in the editor through the existing open command configuration. `copy hunk` copies the active rendered hunk as line-numbered Markdown review context. `open line` opens the current rendered File Detail row's new-file line, and `copy line` copies that row's `path:line` anchor. These commands stay inside the selected file and preserve Review Scope, filters, notes, progress, and task state. `cr.ui.file_detail_navigation` owns hunk header detection, target scroll calculation, changed-row detection/navigation, active hunk line resolution, active hunk extraction, current row new-line resolution, rendered text find, and repeated match navigation. `cr.ui.selected_file_actions` owns selected hunk/line open/copy workflows, while `BrowserCommandExecutor` keeps page checks, cached File Detail line retrieval, status feedback, and redraw control.
