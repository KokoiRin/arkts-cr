...
class BrowserCommandAction:
    ...
    SELECT_SOURCE_SYMBOL = "select_source_symbol"
    ...
    VIEW_TASK_PROBLEM_DIFF = "view_task_problem_diff"
    SAVE_DIFF = "save_diff"
...
def parse_browser_command(command: str, *, raw_keys: bool = False) -> BrowserCommand:
    ...
    if command in {"source select symbol", "select source symbol", "source symbol"}:
        return BrowserCommand(BrowserCommandAction.SELECT_SOURCE_SYMBOL)
    ...
    if command.startswith("source select "):
        return BrowserCommand(
            ...
            command.removeprefix("source select ").strip(),
        )
    ...
    if command in {"copy file problems", "copy problem file", "copy task file problems"}:
        return BrowserCommand(BrowserCommandAction.COPY_FILE_TASK_PROBLEMS)
    ...
    if command in {"next problem file", "problem file next", "next problems file"}:
    ...
    if command in {"problems clear find", "problems clear query", "clear problem find"}:
        return BrowserCommand(BrowserCommandAction.CLEAR_TASK_PROBLEM_QUERY)
    if command.startswith("problems find "):
        return BrowserCommand(
            BrowserCommandAction.SET_TASK_PROBLEM_QUERY,
            command.removeprefix("problems find ").strip(),
        )
    ...
    if (
        command.startswith("save problem ")
        and command != "save problem context"
        and not command.startswith("save problem context ")
    ):
        return BrowserCommand(
            BrowserCommandAction.SAVE_TASK_PROBLEM,
            command.removeprefix("save problem ").strip(),
        )
