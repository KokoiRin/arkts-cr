...
class TaskOutputFindCommandTests(unittest.TestCase):
    def test_browser_command_executor_finds_text_in_task_output(self):
        # Behavior: 当用户在Task Panel / Task Output中输出「BrowserCommandExecutor finds text in Task Output」时，系统应正确更新任务状态、输出、问题和历史记录 [Requirement: TODO]
    ...
    def test_browser_command_executor_repeats_task_output_find_matches(self):
        # Behavior: 当用户在Task Panel / Task Output中输出「BrowserCommandExecutor repeats Task Output find 匹配」时，系统应正确更新任务状态、输出、问题和历史记录 [Requirement: TODO]
        from cr.ui.browser import parse_browser_command
    ...
    def test_browser_command_executor_reports_task_output_find_empty_states(self):
        # Behavior: 当用户在Task Panel / Task Output中处理异常「BrowserCommandExecutor 提示 Task Output find 空态 states」时，系统应给出明确反馈，并保持当前状态安全可恢复 [Requirement: TODO]
        from cr.ui.browser import parse_browser_command
