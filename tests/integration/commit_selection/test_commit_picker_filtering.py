...
class CommitPickerFilteringTests(unittest.TestCase):
    ...
    def test_commit_picker_filter_matches_scope_summary_fields(self):
        # Behavior: 当用户在Commit Picker中过滤「Commit Picker 过滤 匹配 范围 summary fields」时，系统应只呈现符合条件的结果，并保持相关选择规则稳定 [Requirement: TODO]
        commits = [
            CommitSummary(
    ...
    def test_commit_picker_filter_commands_are_isolated_from_file_filter(self):
        # Behavior: 当用户在Commit Picker中过滤「Commit Picker 过滤 命令 are isolated from 文件 过滤」时，系统应只呈现符合条件的结果，并保持相关选择规则稳定 [Requirement: TODO]
    ...
    def test_commit_picker_search_keeps_the_changed_file_filter(self):
        # Behavior: 当用户在Commit Picker中过滤「Commit Picker search keeps the Changed Files 过滤」时，系统应只呈现符合条件的结果，并保持相关选择规则稳定 [Requirement: TODO]
        args = argparse_namespace(
            color="never",
            links="file",
            staged=False,
