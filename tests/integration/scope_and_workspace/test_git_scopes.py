...
class GitScopeTests(unittest.TestCase):
    ...
    def test_git_all_changes_marks_mixed_staged_and_unstaged_sources(self):
        # Behavior: 当用户在Review Scope 与工作区中标记「Git 全部 changes 标记 mixed 已暂存 and 未暂存 sources」时，系统应完成对应产品行为，并保持页面状态正确 [Requirement: TODO]
    ...
    def test_git_local_scopes_mark_staged_and_unstaged_sources(self):
        # Behavior: 当用户在Review Scope 与工作区中标记「Git 本地 范围 标记 已暂存 and 未暂存 sources」时，系统应完成对应产品行为，并保持页面状态正确 [Requirement: TODO]
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
    ...
    def test_git_recent_commits_include_change_summary(self):
        # Behavior: 当用户在Review Scope 与工作区中执行操作「Git 最近 commit 包含 change summary」时，系统应产出正确的结构化结果 [Requirement: TODO]
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            first = repo / "first.ts"
    ...
    def test_git_comparison_scopes_do_not_mark_local_sources(self):
        # Behavior: 当用户在Review Scope 与工作区中标记「Git comparison 范围 do 不 标记 本地 sources」时，系统应完成对应产品行为，并保持页面状态正确 [Requirement: TODO]
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            sample = repo / "Sample.ts"
